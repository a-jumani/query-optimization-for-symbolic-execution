...
'''
Records time spent in forking while solving some path contraints of the GCD function.
...
def test_fork(reps):
	...
	for i in range(reps):
		...
		if pid:
			...
			# adjustments for constraints of next depth
			x, y = y, x % y
...
'''
Records time spent in copying solver while solving some path contraints of the GCD function.
...
def test_copy(reps):
	...
	for i in range(reps):
		...
		solver.add(x % y != 0)
		solver.check()
		s_new.add(x.translate(c) % y.translate(c) == 0)
		s_new.check()
